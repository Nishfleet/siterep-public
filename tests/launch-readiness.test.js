...
test("release status marker is no older than 60 days", async () => {
  ...
  const match = worker.match(/const RELEASE_STATUS_MARKER_UPDATED_AT = "(\d{4}-\d{2}-\d{2})";/);
  assert.ok(match, "worker/index.js must define RELEASE_STATUS_MARKER_UPDATED_AT");

  const [, markerDateString] = match;
